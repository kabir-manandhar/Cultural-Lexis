#!/usr/bin/env python3
"""
Hybrid-distance tension-set analysis – ZH composite plots
========================================================
Produces:

• tension_set_top50.csv                (global US-CN gap)
• <model>_top50.json                   (per-model metrics)
• ZH_qwen_vanilla_vs_sft_shift.pdf     (vanilla ↔ SFT, Qwen)
• ZH_llama_vanilla_vs_sft_shift.pdf    (vanilla ↔ SFT, Llama)
"""
...
def truncate(a: List[float], b: List[float]) -> tuple[List[float], List[float]]:
    """Return vectors truncated to their common minimum length."""
    m = min(len(a), len(b))
    return a[:m], b[:m]

# ---------------------------------------------------------------------------
# Phase 1 – global top-50 tension set
# ---------------------------------------------------------------------------
...
def write_vector_csv(tension_ids: set[str], dfs: dict[str, pd.DataFrame]) -> None:
    """
    Create a CSV with, for each high-divergence question:
        Id, question text,
        us_vec, cn_vec,
        qwen_vanilla, llama_vanilla, qwen_sft, llama_sft
    """
    df_gold = pd.read_json(SURVEY_JSON)

    # Helper: fetch a model vector for a given question Id
    def model_vec(tag: str, qid: str) -> list[float]:
        row = dfs[tag].loc[dfs[tag].Id == qid, "model_vec"]
        return row.iloc[0] if not row.empty else []

    rows = []
    for qid in sorted(tension_ids):                    # ascending order
        ...
        rows.append({
            ...
            "qwen_vanilla":   json.dumps(model_vec("WVS_ZH_qwen_vanilla",  qid)),
            "llama_vanilla":  json.dumps(model_vec("WVS_ZH_llama_vanilla", qid)),
            "qwen_sft":       json.dumps(model_vec("WVS_ZH_qwen_sft",      qid)),
            "llama_sft":      json.dumps(model_vec("WVS_ZH_llama_sft",     qid)),
        })

    df_out = pd.DataFrame(rows)
    out_csv = ROOT_WRITE / "tension_set_vectors.csv"
    df_out.to_csv(out_csv, index=False)
    print(f"✔ probability-vector table → {out_csv.name}")

# ---------------------------------------------------------------------------
# Phase 2 – model-specific DataFrame (always 50 rows)
# ---------------------------------------------------------------------------
def model_dataframe(model_json: Path, tension_ids: set[str]) -> pd.DataFrame:
    df_model = pd.read_json(model_json)
    df_gold  = pd.read_json(SURVEY_JSON)

    df = df_gold[df_gold.Id.isin(tension_ids)].copy()               # keep 50 ids
    df["model_vec"] = df_model.loc[df.index, "choice_values"]       # align rows

    # --- build US / CN vectors once -------------------------------------
    df["us_vec"] = df.us_score.apply(lambda d: list(d.values()))
    df["cn_vec"] = df.china_score.apply(lambda d: list(d.values()))

    # --- hybrid distances with safe truncation --------------------------
...
# ---------------------------------------------------------------------------
# Composite two-panel scatter (no arrows)
# ---------------------------------------------------------------------------
# ---------------------------------------------------------------------------
# Composite two-panel scatter with ONE shared colour-bar
# ---------------------------------------------------------------------------
...
if __name__ == "__main__":
    print("Rebuilding global top-50 tension set …")
    tension_ids = set(generate_tension_csv().Id)      # always 50 ids

    # ------------------------------------------------------------------ #
    # Prepare the 4 ZH model DataFrames   (each guaranteed 50 rows)
    # ------------------------------------------------------------------ #
    ...
    # ------------------------------------------------------------------ #
    # Composite plots  (vanilla  ↔  SFT)    –   no arrows
    # ------------------------------------------------------------------ #
