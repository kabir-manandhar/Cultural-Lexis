...
from src.cultural_lexis_finetune_llms.pipelines.ppo_further_training.dataset_setup import get_cue_word
...
def get_reward(reward_function, response_text_lst, cue_word_lst, torch_type, torch_device):
    # ref: `my_eval_func` in pipelines.language_model_planning_evaluation.nodes
    
    score = reward_function(response_text_lst=response_text_lst,
                            cue_word_lst=cue_word_lst) # shape array (batch_size,)
    # make it to tensor 
...
class RLOOTrainer(Trainer):
    ...
    def __init__(
        ...
        reward_function,
        accelerator, # ! new added
        train_dataloader, # ! new added
        processed_optimizer, # ! new added
        template_str, # "llama3" or "qwen"
        ...
        accelerator.print(f"=== Total Episodes: {args.total_episodes} ===")
        # accelerator = Accelerator(gradient_accumulation_steps=args.gradient_accumulation_steps) # ! comment out
        self.accelerator = accelerator
        accelerator.print("=== Accelerator Placed ===")
        # args.world_size = accelerator.num_processes # already set in ppo_train.py
        ...
        ### setup dataloader
        #########
        # self.dataloader = DataLoader( # TODO comment from here
        #     self.train_dataset,
        ...
        # self.model, self.optimizer, self.dataloader = accelerator.prepare(self.model, self.optimizer, self.dataloader) # TODO may change this 
        self.optimizer = processed_optimizer
    ...
    def train(self):
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            with torch.no_grad():
                ...
                # * --- add `cue_words`
                cue_words = processing_class.batch_decode(queries) # type list 
                cue_words = [get_cue_word(self.template_str, q) for q in cue_words]
                cue_words = cue_words * args.rloo_k # repeat the cue_words for rloo_k times
                ...
                for i in range(0, queries.shape[0], args.local_rollout_forward_batch_size):
                    ...
                    # * --- add `cue_words`
                    cue_words_lst = cue_words[i : i + args.local_rollout_forward_batch_size]
                    ...
                    response_text_lst = processing_class.batch_decode(postprocessed_response, skip_special_tokens=True)
                    
                    
                    score = get_reward(
                        reward_function, response_text_lst, cue_words_lst, logprob.dtype, query.device
                    )
    ...
    def generate_completions(self, sampling: bool = False):
        ...
        with unwrap_model_for_generation(self.model, self.accelerator) as unwrapped_model:
            for batch in self.eval_dataloader:
                ...
                # * --- add `cue_words`
                cue_words = processing_class.batch_decode(query) # type list
                cue_words = [get_cue_word(self.template_str, q) for q in cue_words]
                with torch.no_grad():
                    ...
                    score = get_reward(
                        self.reward_function, response_text_lst, cue_words, torch.float32, query.device)
